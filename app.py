...
st.set_page_config(page_title="DOC Hesaplayıcı (SKU Bazlı) — Streamlit", layout="wide")
...
if run and uploaded_file is not None:
    # Colab'daki okuma ile eşdeğer (in-memory)
    df = pd.read_excel(uploaded_file)

    # Month columns yakalar (datetime tipindeki)
    months_columns = [c for c in df.columns if isinstance(c, (pd.Timestamp, _dt))]
    months_columns.sort()

    with st.expander("Diagnostik (2. Blok)"):
        st.write("Months columns (ilk 6):", months_columns[:6], "… toplam:", len(months_columns))

    # key figure kategorilerini yazdırmak
    df["_kf_class"] = df[kf_col].map(classify_kf)

    # ⚠️ ÖNEMLİ: consensus satırlarını EIP'e çeviren satır KALDIRILDI (orijinal talebe uygun)
    # df.loc[df["_kf_class"] == "consensus", "Plant"] = "EIP"

    # ✅ EŞLEŞME TESTİ – Gözle kontrol için
    df["_key_figure_normalized"] = df[kf_col].map(norm_text)

    with st.expander("Key Figure Eşleştirme Kontrolü"):
        st.write("Unique eşleşmeler:")
        st.dataframe(df[["_kf_class", kf_col]].drop_duplicates(), use_container_width=True)
        st.write("İçinde 'consensus' geçen normalized satırlar:")
        st.dataframe(
        ...
        st.write("Consensus satırlarının Plant dağılımı (kontrol amaçlı):")
        st.write(_consensus_plant_counts)

    # ===================== 3. BLOK =====================
    import datetime as _dt2  # orijinal isimlendirme korunuyor
    ...
    def detect_month_columns_flexible(df_in: pd.DataFrame):
        ...
        for c in df_in.columns:
            if isinstance(c, (pd.Timestamp, _dt2.datetime)):
                ts = pd.Timestamp(c)
    ...
    st.success("Hazır: sku_doc_res (ürün bazlı DOC, konsensus sadece EIP'e ait kodlardan) ve total_monthly (toplam).")

    # ===================== 4. BLOK — Çıktılar (Streamlit'e uygun) =====================
    # DOC_by_PRODUCT.xlsx
    cols_order = [
    ...
    st.write("**Ürün Bazlı (ilk 200 satır):**")
    st.dataframe(out_df.head(200), use_container_width=True)
    st.write("**Toplam (ilk 200 satır):**")
    st.dataframe(total_monthly_reset.head(200), use_container_width=True)

else:
    st.info("Başlamak için bir .xlsx dosyası yükleyin ve **Hesapla** butonuna basın.")
